...
const Dashboard: React.FC = () => {
  ...
  const quickStats = isFreelancer
    ? [
        { label: 'Active Listings', value: '—', icon: <Briefcase size={18} />, color: 'brand-green' },
        { label: 'Avg. Rating', value: '—', icon: <Star size={18} />, color: 'brand-pink' },
        { label: 'Total Earned', value: '€—', icon: <TrendingUp size={18} />, color: 'brand-green' },
        { label: 'Orders Completed', value: '—', icon: <Zap size={18} />, color: 'brand-pink' },
      ]
    : [
        { label: 'Active Jobs', value: '—', icon: <Briefcase size={18} />, color: 'brand-green' },
        { label: 'Open Orders', value: '—', icon: <Clock size={18} />, color: 'brand-pink' },
        { label: 'Total Spent', value: '€—', icon: <TrendingUp size={18} />, color: 'brand-green' },
        { label: 'Hired Freelancers', value: '—', icon: <User size={18} />, color: 'brand-pink' },
      ];

  const quickActions = isFreelancer
    ? [
        { label: 'Create Listing', icon: <Plus size={16} />, to: '/profile', desc: 'Post a new service' },
        { label: 'View Orders', icon: <Briefcase size={16} />, to: '/orders', desc: 'Track active work' },
        { label: 'Messages', icon: <MessageSquare size={16} />, to: '/messages', desc: 'Client conversations' },
        { label: 'My Profile', icon: <User size={16} />, to: '/profile', desc: 'Edit your portfolio' },
      ]
    : [
        { label: 'Post a Job', icon: <Plus size={16} />, to: '/profile', desc: 'Find top talent' },
        { label: 'Browse Market', icon: <Zap size={16} />, to: '/marketplace', desc: 'Explore services' },
        { label: 'My Orders', icon: <Briefcase size={16} />, to: '/orders', desc: 'Track projects' },
        { label: 'Messages', icon: <MessageSquare size={16} />, to: '/messages', desc: 'Talk to freelancers' },
      ];
  ...
  return (
    <div className="relative min-h-screen pt-12 pb-24 px-4 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-12">
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-green mb-2">Welcome back</p>
        ...
        {!isVerified && (
          <div className="mb-10 p-5 bg-yellow-500/10 border border-yellow-500/20 rounded-2xl flex items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-white font-bold text-sm">Identity verification required</p>
                <p className="text-gray-400 text-xs mt-0.5">Complete KYC to unlock listings, orders, and full platform access.</p>
              </div>
            </div>
            <Link
        ...
        {isVerified && (
          <div className="mb-10 p-5 bg-brand-green/10 border border-brand-green/20 rounded-2xl flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400 text-xs mt-0.5">Your account has full platform access.</p>
            </div>
          </div>
        )}
        ...
        {/* Quick Actions */}
        <div className="mb-10">
          <h2 className="text-xs font-black text-gray-600 uppercase tracking-[0.2em] mb-5">Quick Actions</h2>
        ...
        <div>
          ...
          <div className="bg-brand-grey/40 border border-white/5 rounded-2xl p-10 text-center">
            ...
            <p className="text-gray-600 font-bold text-sm">No recent activity yet</p>
            <p className="text-gray-700 text-xs mt-1">Your orders and interactions will appear here.</p>
            <Link
              to="/marketplace"
